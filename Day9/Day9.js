...
function OASIS(input) {
    ...
    const part1Val = routes.map(route =>{
        route.reverse();
        return(route.reduce((a,b)=> {
            a = Array.isArray(a) ?  a[a.length-1] : a
            return a + b[b.length-1];
        }))
    });
    const part2Val = routes.map(route =>{
        route.reverse();
        return(route.reduce((a,b)=> {
    ...
    console.log(`Part 1 = ${part1Val.reduce((a,b)=> a + b)}`);
    console.log(`Part 2 = ${part2Val.reduce((a,b)=> a + b)}`);
}
