...
function getValues(string) {
    return string.split(':')[1].split(' ') .filter((s) => s !== '').map((ele)=>Number.parseInt(ele));
}

function race(str) {
    console.log(str);
    const time = getValues(str.split("\r\n")[0]);
    const distance = getValues(str.split("\r\n")[1]);
    console.log(time);
    ...
    for(let i = 0; i<time.length; i++) {
        ...
        let raceTime = time[i];
        let minimum = distance[i] - time[i];
        let canBeatRecord = 0;
        for(let pressing = 0; pressing<time[i]; pressing++) {
            const rolling = raceTime - pressing;
